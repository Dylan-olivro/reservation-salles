...
// $requete = mysqli_query($connexion, $reponse);
// $login = $reponse['login'];
// $login= $resultat['login'];

// $reponse = mysqli_query($connexion, 'SELECT reservations.titre, utilisateurs.login  FROM utilisateurs INNER JOIN reservations ON utilisateurs.id = reservations.id_utilisateur');
...
// boucle pour la ligne des jours de la semaine
	...
	// $id=$value[0];
					$jour=date("w", strtotime($value[6]));
					$heure=date("H", strtotime($value[6]));
				
					if($heure==$ligne && $jour== $colonne)
						{
                        echo"$value[1]<br>$value[4]";
